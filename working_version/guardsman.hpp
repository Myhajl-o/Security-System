class guardsman
{
      ...
      void getting_key()
      {
            ...
            #else
                  std::cout << "ОС не підтримується!" << std::endl;
                  std::exit(1);
            #endif

            std::ifstream exeFile(exe_path, std::ios::in | std::ios::binary);
            if(!exeFile.is_open()) {
                  std::cout << "Не вдалося відкрити власний процес." << std::endl;
                  std::exit(1);
            }

            exeFile.seekg(0, std::ios::end);
            size_t file_size = exeFile.tellg();

            size_t target_offset = (file_size * 3) / 4;

            exeFile.seekg(target_offset, std::ios::beg);
            secret_key.resize(64);
            exeFile.read(secret_key.data(), secret_key.size());
      ...
      {
            std::filesystem::path a="a";
            std::ifstream text_file(a/"b1_H", std::ios::in | std::ios::binary);
            ...
            text.resize(size_text);
            text_file.read(text.data(),size_text);
            text_file.close();
            XOR(text,secret_key2);
            std::string temp = "16347830079901180040";
      ...
      {     
            ...
            //if(!change_cipher("e")){ std::cout<<"Пробелема під час перезаписування"<<std::endl; std::exit(1); }
